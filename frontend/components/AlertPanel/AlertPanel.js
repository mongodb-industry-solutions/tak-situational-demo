...
// Extract <emergency type="..." > from CoT detail XML
...
function alertColor(w) {
  ...
  return "#f97316"; // orange for b-a-* and anything else active
}

function AlertCard({ alert }) {
  const { type: emergencyType } = extractEmergency(alert.r);
  const color = alertColor(alert.w);
  // c = alert name (e.g. "Alberto-Alert"), e = device operator callsign
  ...
  return (
    ...
    >
      ...
      {alert.e && alert.e !== alert.c && (
        <Body style={{ color: palette.gray.light1, fontSize: "11px" }}>
          Sent by: {alert.e}
        </Body>
      )}
    </div>
